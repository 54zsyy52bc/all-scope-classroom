...
(function (global) {
  ...
  function viewCheckin(state) {
    return '<section class="panel">'
      ...
      + '<label class="field field-seat"><span class="field-label">座位号（组内）</span>'
      + '<input id="f-seat" class="input input-seat" type="text" inputmode="numeric" maxlength="2"'
      + ' autocomplete="off" placeholder="01" value="' + esc(state.seat || '') + '" /></label>'
      + '<label class="field"><span class="field-label">小组号（选填，留空自动）</span>'
      + '<input id="f-group" class="input" type="text" inputmode="numeric" maxlength="3"'
      + ' autocomplete="off" placeholder="3 或 03" value="' + esc(state.group || '') + '" /></label>'
