...
- 监听 `0.0.0.0:3000` 是**硬需求**（学生端桌面会跨机轮询 `/api/v1/system/health` 显示「教室
  在线/离线」，收敛到 `127.0.0.1` 会让全部学生机恒显「教室离线」）。因此安全边界由**鉴权**承担：
  - `ENABLE_AUTH` **默认开启**；教师机本机免鉴权——判定同时覆盖「回环地址」与「本机网卡地址」，
    所以老师无论用 `http://127.0.0.1:3000` 还是 `http://<教师机IP>:3000` 打开大屏都能正常使用。
  - 仅 `/api/v1/system/health` 对局域网公开（只返回运行状态，**不含**业务数据与凭据告警）。
  - 其余跨机调用一律要求 `X-Teacher-Token`（恒定时间比较）。`TEACHER_TOKEN` 为空时启动会生成
    一次性随机令牌并打印到日志（重启失效），故**跨机集成方须在 `app-config.json` 显式设置稳定令牌**。
  - CORS 默认放行「无 Origin（curl/测试）」与「`Origin: null`（Electron `file://` 渲染进程，
    学生端探活即属此类）」与同源；其它站点需加入 `CORS_ORIGINS` 白名单。
